...
- **运维与工具链**：版本检查 + 双后端更新、Factory Reset、备份/恢复、启动自检、完整性签名、插件脚手架与离线安装/卸载 CLI，以及 **49 脚本回归套件与 GitHub Actions CI**。
...
- **天生国际化**：JSON 语言包、模板/后端/前端统一 t()、每个页面都可切换语言，外加全量浅色/深色**主题体系**（跟随系统或手动覆盖）。
...
- **Community Edition（v4.x）**：功能开发持续进行，但架构规模有意识控制——专注小型局域网/个人用户场景，我们定期维护与发布（49 脚本回归套件 + CI）。
...
本项目在开发过程中使用了 AI 辅助编程工具，包括但不限于：代码生成与重构、代码审查、测试用例编写、文档撰写。所有 AI 辅助生成或修改的内容，均已由开发者人工审查，并通过项目自身的回归测试套件（`tests/`，49 脚本 / 1333 项断言）与启动完整性自检验证后才会合入。
