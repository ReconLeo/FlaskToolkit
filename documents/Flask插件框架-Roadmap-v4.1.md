...
| 版本 | 状态 |
|------|------|
| **v4.12.0（当前）** | Secure 安全传输：框架自身 HTTP→HTTPS 自动跳转（core/network.start_http_redirect 308 保留 POST，主端口+1 跳转端口）+ SESSION_COOKIE_SECURE 自动配置（None=自动：HTTPS/反代自动 true、纯 HTTP 自动 false，可显式强制）+ config.py SSL 配对提示与 HTTPS 状态检查 + 桌面启动器 HTTPS 复选框与证书自动生成（ensure_https_cert + --https）+ 反代支持归档（TRUST_PROXY_HEADERS ProxyFix / EXTERNAL_SCHEME / EXTERNAL_HOST / EXTERNAL_PORT / validate_ssl_cert / mDNS 提示插值）；回归 32 脚本 807 项 |
