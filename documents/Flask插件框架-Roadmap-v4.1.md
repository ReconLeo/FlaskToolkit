...
| v4.16（规划中） | 事件总线（core/events.py：广播/订阅/同步异步）+ 插件真依赖解析（plugin.json dependencies 现仅冲突比对 → 安装时解析依赖链/缺依赖处理），为第三方插件市场生态铺路 |
