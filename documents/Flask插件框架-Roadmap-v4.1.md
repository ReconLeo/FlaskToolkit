...
| 版本 | 状态 |
|------|------|
| **v4.14.0（当前）** | Statistics 数据统计洞察：时间桶（daily_stats 按天聚合 count/ok/4xx/5xx/ms）+ 访问画像（access_profile：by_ip 追溯主键 + by_user 登录用户含管理员 + 设备分类 bot/tablet/mobile/desktop）双维数据模型（core/stats.py 重写）；after_request global_stats_recorder 埋点（职责分离防双计数，401/403/404 计入 4xx）；dashboard 总览化（运行徽章行 + 冷门插件提示 + 最近动态卡 + 补网络入口）；统计页 14 天趋势（纯 SVG）+ 错误 Top + 画像卡；STATS_RETENTION_DAYS/ACCESS_PROFILE_ENABLED 配置；跳转端口 POST body 消费修复；新增 test_stats 55 项，回归 33 脚本 869 项 |
...
| v4.15（规划中） | 待定（企业级需求引导至 Enterprise Edition 路线讨论） |
