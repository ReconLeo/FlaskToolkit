...
- [ ] `documents/Flask-Plugin-Framework-Changelog.md`：新增版本小节 + 版本线总览表（**历史演进唯一出处**）；`开发规范` 只同步新章节/目录树（不再写版本标题/版本说明）
- [ ] `README.md`（英文主版）+ `README.zh-CN.md`（中文版）：亮点、断言数、徽章、示例表（**双版本同步**，勿只改一处）
- [ ] `documents/Flask-Plugin-Framework-Roadmap-v4.md`：版本对照表（当前版本行 + 已完成行；注意多行编辑顺序勿覆盖旧行）
...
- [ ] 两文档内容边界：README=门面（精简），开发规范=权威规格（目录树/详细规格唯一出处）；**历史演进唯一出处=版本演进记录.md**，开发规范不重复演进史（仅顶部保留演进记录链接）
