...
| **v4.17.2（当前）** | AirDrop 框架协调 A+C 组：冲突报错附两处值（plugin_pack.py）+ 描述文件失效打标与后台徽章（plugin_cache/plugin_loader/plugins.html）+ 源码布局自动映射 --src-layout（tools/package.py + Dev-Guide 5.6.9）+ CRLF 规范提示；新增 test_src_layout 16，扩展 test_pack_meta 23 + test_meta_e2e 11，全量回归 44 脚本 1161 项，tag v4.17.2 已发布 |
