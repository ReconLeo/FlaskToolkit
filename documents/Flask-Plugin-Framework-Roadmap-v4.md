...
> 本文档梳理框架的提升方向：v4.1 稳定版前（P0 安全/数据完整性 → P1 测试/工程化 → P2 体验/可维护性）与 **v4.3 安全大更新**（见第六章）。每项标注落地状态，具体开发规范见《Flask-Plugin-Framework-Dev-Guide-v4.md》。
...
| 版本 | 状态 |
|------|------|
| **v4.15.0（已完成）** | Root 域与市场骨架：framework 能力域三档 read/manage/core（core≈Linux root，filesystem:write 核心收紧 + check_framework + root-access 审计）→ 程序化插件管理服务层 core/plugin_admin.py（require_manage/scan_gate/安装·更新·卸载·启用·禁用·purge，为第三方市场铺路）→ 插件级更新源 core/plugin_updates.py（plugin.json repo/update_feed + feed JSON + RSA 验签 + 缓存 + 3s 静默，后台插件页检查更新/更新徽章）→ 前端 Root/Manage/Framework 徽章 + 三色警示条 + preview capabilities 恒空 bug 修复 → selfcheck 时区探测（global_var.TIMEZONE + zoneinfo，缺 tzdata 自检致命报错）与 CORE_FILES 补全 → requirements 增 tzdata==2026.3；新增 test_root_domain 18 项 + test_selfcheck 14 项，回归 35 脚本 914 项 |
| v4.14.0（已完成） | Statistics 数据统计洞察：时间桶 + 访问画像双维数据模型 / dashboard 总览化 / 14 天趋势 + 错误 Top + 画像卡 / 跳转端口 POST body 消费修复；新增 test_stats 55 项，回归 33 脚本 869 项 |
| v4.13.0（已完成） | Mobile & Tablet 移动端与平板适配：公开页面 + 后台管理页响应式翻修（static/css/mobile.css + admin-mobile.css 与原有样式分开创建，支持刘海屏安全区 viewport-fit=cover + 44px 触摸目标 + 480/768 断点）+ JS 增强层（static/js/mobile.js 四件套：表格自动包裹 .table-scroll / 首页汉堡菜单 / 窄屏模态框全屏 / toast 顶部通栏，MutationObserver 兜底）+ 14 框架模板幂等注入（viewport-fit + link/script）+ 五个示例插件各自 *_mobile.css（11 模板引入）；浏览器端到端验证通过，回归 32 脚本 869 项 |
| **v4.18.0（当前）** | B 组同步持久化上传助手：BasePlugin 新增 upload_dir + sanitize_filename + save_uploads（单/多文件·净化·去重·大小+配额双预检·直接落盘，部分成功语义）；AirDrop upload_files/get_safe_filename 迁移；新增 test_plugin_uploads 21，全量回归 45 脚本 1182 项，tag v4.18.0 已发布 |
| **v4.17.2（已完成）** | AirDrop 框架协调 A+C 组：冲突报错附两处值（plugin_pack.py）+ 描述文件失效打标与后台徽章（plugin_cache/plugin_loader/plugins.html）+ 源码布局自动映射 --src-layout（tools/package.py + Dev-Guide 5.6.9）+ CRLF 规范提示；新增 test_src_layout 16，扩展 test_pack_meta 23 + test_meta_e2e 11，全量回归 44 脚本 1161 项，tag v4.17.2 已发布 |
| **v4.17.1（已完成）** | 签名功能验证 + 修复自更新/插件更新源验签 bug（update_checker/plugin_updates._verify_feed_signature 漏传 signature，配公钥后签名永远失败）：新增 test_plugin_updates 8 + test_release_sign 5，扩展 test_package_sign 25 + test_update_checker 50，全量回归 43 脚本 1143 项，tag v4.17.1 已发布 |
| **v4.17.0（已完成）** | 移动端/桌面端页面分离：core/device.py（UA 检测 detect_device/get_device/is_mobile + resolve_template/render 分发 + MOBILE_ENABLED/FORCE_MOBILE 配置）→ 框架公开页移动端独立模板 templates/mobile/login.html·index.html + static/css/mobile-app.css 精简布局层（routes/public.py 登录/首页 device.render 分发，错误页沿用响应式）→ 后台 _admin_page device.render 分发挂点（开放 mobile/admin/<template>，后台响应式保底）→ BasePlugin 移动端能力（_resolve_template(mobile)/is_mobile_context/mobile_template/render·render_plugin_page·子页面自动分发 plugins/<name>/mobile/）→ corp_tools 1.1.0 移动端独立模板演示（4 同名模板脱离 corp_mobile.css，require 4.17.0）；新增 test_device 20 项，回归 41 脚本，tag v4.17.0 已发布 |
| **v4.16.0（已完成）** | 事件总线 + 插件真依赖解析：自研 core/events.py（发布-订阅 on/once/off/emit/has/clear，weakref 防泄漏，async_=True 后台线程池，插件自定义事件 plugin:<name>:<event>）+ BasePlugin 事件集成（on_event/emit_event/event_name/_cleanup_events，订阅卸载/禁用自动清理）→ 插件真依赖解析 core/plugin_deps.py（dependencies/pip_dependencies 版本约束 name>=x，parse_dep_spec/version_satisfies/Kahn 拓扑+环检测，缺失/版本/循环标记『未加载』+后台透出原因）→ 卸载反向依赖检查 + 安装依赖缺失告警 → scheduler_demo 1.2.0·dependent_demo 2.0.0 事件演示（跨插件事件订阅）；新增 test_events 11 + test_dependency 11 + test_plugin_events 28，回归 40 脚本，tag v4.16.0 已发布 |
| v4.12.2（已完成） | 安全修复：上传临时文件防线——F6 失败分支统一清理（preview/confirm 失败残留）+ preview 文件 TTL 30min 防写盘累积 + F12 preview_id 路径穿越封堵（严格 uuid 格式校验）+ test_admin_api 62→69 项 |
...
> 2026-09-05：Community Edition 功能开发接近终点（v4.8.0 后功能冻结），远期规划移交 **Enterprise Edition（v5.x）**，公开寻求接手。交接声明、架构地图与接手指引见 `documents/Enterprise-Edition-Handover-Roadmap.md`。
