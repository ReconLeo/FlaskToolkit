...
- On top of that "bare trust" model, the framework layers defense in depth:
  1. **AST static scanning** (4.3.1) — blocks obviously risky code at install time. Most high-severity behaviors (eval/subprocess/deserialization/obfuscation/dynamic-import/socket-server/syntax) are *unconstrainable* and always blocked under `enforce`; `shutil.rmtree` is a *constrainable* high exempted only when its constant path is covered by a declared `filesystem:write:<dir>`, or the author marks the line with a `# scan:ignore` / `# scan:ignore:rmtree` escape-hatch comment (an explicit informed declaration — `enforce` degrades to an informed-declaration gate; see dev guide §10.6). Community takes this path; Enterprise-tier deep security should use signer-endorsement / admin-confirmed allow-lists instead;
